...
elevation <- read.table("elevation.dat", skip = 0, header = FALSE)
names(elevation) <- c("x", "y", "z")
library(scatterplot3d) # Add-on package with 3d plotting functions
scatterplot3d(elevation, type = "h",
              xlab = "SRS Easting (ft)", ylab = "SRS Northing (ft)",
              zlab = "Elevation (ft)",
              main = "F-area Elevation Data")
dev.print(pdf, file = "elevpoints.pdf", height = 8, width = 8)
...
nll <- function(param, Xmat, d, z, verbose = FALSE){
  beta <- param[1:3]; sigma2 <- param[4]; rho <- 1 #param[5]
  mu <- Xmat %*% beta
...
Xmat <- cbind(1, elevation$x, elevation$y)
d <- as.matrix(dist(elevation[,1:2]))

## Choose starting values for the minimization

beta.ols <- lm(elevation$z~Xmat-1)$coef # OLS regression; assuming iid errors
names(beta.ols) <- NULL
...
op <- optim(start, fn = nll, method = "L-BFGS-B",
            ...
            Xmat = Xmat, d = d, z = elevation$z, verbose = TRUE)
...
sapply(1:length(mle), function(i){
  ...
  nll.seq <- apply(values, 1, nll, Xmat = Xmat, d = d, z = elevation$z)
...
gridn <- 40
xseq <- seq(m1x, m2x, length = gridn)
yseq <- seq(m1y, m2y, length = gridn)
krige.grid <- expand.grid(x = xseq, y = yseq)

## Kriging using estimated parameters

dpred <- as.matrix(dist(rbind(elevation[,1:2], krige.grid)))[1:85,-(1:85)]
Xpred <- cbind(1, krige.grid$x, krige.grid$y)
g <- exp(-dpred/mle["rho"])
G <- exp(-d/mle["rho"])
zpred <- Xpred %*% mle[1:3] + t(g) %*% solve(G, elevation$z - Xmat %*% mle[1:3])

## Plotting - restrict to convex hull of the data

library(fields) # Add-on package for spatial stats
zmat <- matrix(zpred, nrow = 50)
ok <- in.poly(krige.grid, elevation[,1:2], convex.hull = TRUE)
zmat[!ok] <- NA
